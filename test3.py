...
results = np.load(
    "./priorMDM/save/my_humanml_trans_enc_512/DoubleTake_samples_my_humanml_trans_enc_512_000200000_seed10_handshake_20_double_take_blend_10_skipSteps_100/results.npy",
    allow_pickle=True,
).item()
...
for i in tqdm(range(0, 32, 2)):
    ...
    for j, joint in enumerate(joints):
        balls[j].mlab_source.set(
            x=motion[joint[0]][2][i], y=motion[joint[0]][0][i], z=motion[joint[0]][1][i]
        )
...
prompt = "animated man in suit jumping on a beach, best quality, masterpiece"
...
with torch.no_grad():
    ...
    latents = latents.repeat(1, 1, 1, 1, 1)
    ...
    video = upscale(video)
    imageio.mimsave(f"result_upscaled.mp4", video, fps=8, codec="libx264")
